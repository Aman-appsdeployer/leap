...
def attempt_quiz(
    ...
    try:
        ...
        assignment = db.execute(text("""
            SELECT id FROM Batch_Assignment
        ...
        db.execute(text("""
            INSERT INTO Attempt (batch_assignment_id, student_id_fk, attempt_type, attempt_date)
            VALUES (:assignment_id, :student_id, :attempt_type, NOW())
        """), {
        ...
        return {"message": f"{attempt_type.capitalize()} Quiz Attempt Recorded."}
...
# @quiz_router.get("/api/quiz/{quiz_id}")
# def get_quiz_by_id(quiz_id: int, db=Depends(get_db)):
#     try:
#         quiz = db.execute(text("SELECT * FROM Quiz WHERE quiz_id = :quiz_id"), {"quiz_id": quiz_id}).fetchone()
#         if not quiz:
...
# @quiz_router.put("/api/quiz/{quiz_id}")
# def update_quiz(quiz_id: int, data: FullQuizCreate, db=Depends(get_db)):
#     existing = db.execute(text("SELECT * FROM Quiz WHERE quiz_id = :quiz_id"), {"quiz_id": quiz_id}).fetchone()
#     if not existing:
#         raise HTTPException(status_code=404, detail="Quiz not found.")
...
# @quiz_router.delete("/api/quiz/{quiz_id}")
# def delete_quiz(quiz_id: int, db=Depends(get_db)):
#     quiz = db.execute(text("SELECT * FROM Quiz WHERE quiz_id = :quiz_id"), {"quiz_id": quiz_id}).fetchone()
#     if not quiz:
#         raise HTTPException(status_code=404, detail="Quiz not found.")
