...
def attempt_quiz(payload: dict = Body(...), db=Depends(get_db)):
    try:
        ...
        attempt_type = payload.get("attempt_type")
...
# #     try:
# #         result = db.execute(text("""
# #             SELECT q.quiz_id, q.quiz_title, q.description, q.is_open,
# #                 (
# #                     SELECT COUNT(*) 
# #                     FROM Attempt a
# #                     JOIN Batch_Assignment ba2 ON a.batch_assignment_id = ba2.batch_assignment_id
...
#         # ✅ Improved Field Validation with Clear Messages
#         if not student_id:
...
#             raise HTTPException(status_code=400, detail="Invalid attempt_type (must be 'pre' or 'post')")

#         # ✅ Fetch assignment to ensure the student is assigned the quiz
#         assignment = db.execute(text("""
#             SELECT batch_assignment_id FROM Batch_Assignment
#             WHERE student_id_fk = :student_id AND quiz_id_fk = :quiz_id
#         """), {"student_id": student_id, "quiz_id": quiz_id}).fetchone()
...
#         return {"message": f"{attempt_type.capitalize()} Quiz Attempt Recorded. Attempt #{attempt_number}"}

#     except Exception as e:
#         db.rollback()
#         import traceback
#         print("❌ ATTEMPT ERROR:", traceback.format_exc())
#         raise HTTPException(status_code=500, detail=f"❌ Error recording attempt: {str(e)}")
...
#     """), {"quiz_id": quiz_id, "student_id": student_id}).fetchone()

#     return {"attempt_count": result.count if result else 0}
