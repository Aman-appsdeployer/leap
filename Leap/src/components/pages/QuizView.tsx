...
const QuizView = () => {
  ...
  const fetchAttemptCount = async (quizId: number) => {
    try {
      ...
      if (studentData) {
        ...
        if (response.data.attempt_count) {
          const count = response.data.attempt_count;
          setAttemptCount(count);
          if (count >= 3) setMaxReached(true);
        } else {
          console.error("Attempt count data not returned properly");
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    quiz.questions.forEach((q: any) => {
      ...
      if (correct?.option_id === selectedOptionId) totalScore += 1;
    });
    ...
    try {
      ...
      const studentId = studentData.student_details_id_pk || studentData.student_id;
      const attemptType = location.state?.attempt_type || "post";

      // Only record the attempt when the quiz is submitted
      ...
      await fetchAttemptCount(quiz.quiz_id); // Refresh attempt count
    } catch (error) {
  ...
  // Handle the scenario when the quiz data is not loaded
  if (!quiz) return <div className="p-8 text-red-600">❌ Quiz not loaded. Please go back and try again.</div>;
  ...
  return (
    <div className="min-h-screen p-8 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white">
      <div className="max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
        ...
        {maxReached && <div className="text-red-600 font-semibold mb-4">❌ Max 3 attempts reached.</div>}

        {!maxReached && quiz.questions.map((q: any) => (
